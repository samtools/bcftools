...
static AlignmentData *CreateAlignmentData(int maxNumBlocks, int targetLength) {
    AlignmentData *d = malloc(sizeof(*d));
    ...
    d->Ps     = malloc(maxNumBlocks * targetLength * sizeof(*d->Ps));
    d->Ms     = malloc(maxNumBlocks * targetLength * sizeof(*d->Ms));
    d->scores = malloc(maxNumBlocks * targetLength * sizeof(*d->scores));
    d->firstBlocks = malloc(targetLength * sizeof(*d->firstBlocks));
    d->lastBlocks  = malloc(targetLength * sizeof(*d->lastBlocks));

    return d;
}

static void DestroyAlignmentData(AlignmentData *d) {
    free(d->Ps);
    free(d->Ms);
    free(d->scores);
    free(d->firstBlocks);
    ...
    if (result.editDistance >= 0) {  // If there is solution.
        ...
        if (config.task == EDLIB_TASK_PATH) {
            ...
            obtainAlignment(query, rQuery, queryLength,
                            ...
                            equalityDefinition, alphabet_size, result.editDistance,
                            &(result.alignment), &(result.alignmentLength));
            free((void *)rAlnTarget);
            free((void *)rQuery);
        }
#endif
    }
...
char* edlibAlignmentToCigar(const unsigned char* const alignment, const int alignmentLength,
    ...
    char* cigar_ = malloc(cigar->size() * sizeof(char));
        ...
        for (int b = 0; b < maxNumBlocks; b++) {
            if (symbol < alphabetLength) {
                ...
                for (int r = (b+1) * WORD_SIZE - 1; r >= b * WORD_SIZE; r--) {
                    ...
                    if (r >= queryLength || equalityDefinition_areEqual(equalityDefinition, query[r], symbol))
                        Peq[symbol * maxNumBlocks + b] += 1;
                }
 ...
 *                         Make sure to free this array using free().
...
static int obtainAlignmentTraceback(const int queryLength, const int targetLength,
    ...
    *alignment = malloc((queryLength + targetLength - 1) * sizeof(unsigned char));
    *alignmentLength = 0;
    ...
    *alignment = realloc(*alignment, (*alignmentLength) * sizeof(unsigned char));
...
static int obtainAlignment(
    ...
    if (queryLength == 0 || targetLength == 0) {
        ...
        *alignment = malloc((*alignmentLength) * sizeof(unsigned char));
        for (int i = 0; i < *alignmentLength; i++) {
...
static int obtainAlignmentHirschberg(
    ...
    if (leftHalfCalcStatus == EDLIB_STATUS_ERROR || rightHalfCalcStatus == EDLIB_STATUS_ERROR) {
        if (alignDataLeftHalf)  free(alignDataLeftHalf);
        if (alignDataRightHalf) free(alignDataRightHalf);
        return EDLIB_STATUS_ERROR;
    }
    ...
    int* scoresLeft = malloc(scoresLeftLength * sizeof(int));
    ...
    int* scoresRight = malloc(scoresRightLength * sizeof(int));
    ...
    free(alignDataLeftHalf);
    free(alignDataRightHalf);
