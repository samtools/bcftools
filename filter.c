...
void filter_expression_info(FILE *fp)
{
    ...
    fprintf(fp, "    - array subscripts, such as (e.g. AC[0]>=10)\n");
    fprintf(fp, "    - double quotes for string values (e.g. %%FILTER=\"PASS\")\n");
    fprintf(fp, "    - 1 (or 0) for testing the presence (or absence) of a flag (e.g. FlagA=1 && FlagB=0)\n");
    fprintf(fp, "    - TAG or INFO/TAG for INFO values (e.g. DP<800 or INFO/DP<800)\n");
    fprintf(fp, "    - %%QUAL, %%FILTER, etc. for column names (note: currently only some columns are supported)\n");
    fprintf(fp, "    - %%TYPE for variant type, such as %%TYPE=\"indel\"|\"snp\"|\"mnp\"|\"other\"\n");
    fprintf(fp, "    - %%FUNC(TAG) where FUNC is one of MAX, MIN, AVG and TAG is one of the FORMAT fields (e.g. %%MIN(DV)>5)\n");
}
